  ...
  return (
    <div className={`min-h-screen bg-[var(--bg-primary)] dark:bg-card-gradient transition-colors duration-300 ${className}`}>
      ...
      <PageContainer className="max-w-5xl mx-auto px-4 py-12 sm:py-16 lg:py-20">
        ...
        {/* Footer */}
        <footer className="mt-20 text-center text-[var(--text-muted)] text-sm pb-8 flex flex-col items-center gap-4">
          <a 
            href="https://github.com/DomeenoH/eol-dr" 
            target="_blank" 
            rel="noopener noreferrer" 
            className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--bg-surface)] hover:bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-full transition-all duration-200 hover:-translate-y-0.5 group"
            aria-label="访问 GitHub 仓库"
          >
            <GitHubIcon className="w-5 h-5 text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition-colors" />
            <span className="font-medium text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">GitHub Repository</span>
          </a>
          <p>
            源自开源项目 <span className="font-medium">EOL DR</span>
          </p>
        </footer>
      </PageContainer>
    </div>
